...
class Panel(View):
    ...
    async def search_game(self, interaction, _):
        ...
        # Publicamos UNA sola vez: en el canal correspondiente al rango del jugador.
        # La búsqueda de rangos se mantiene para las menciones: se avisa a los
        # tres rangos que pueden encajar con el jugador.
        search_channel_id = TIER_CHANNELS.get(solo_tier)
        channel = (
            interaction.guild.get_channel(search_channel_id)
        ...
        if not channel:
            print(
                f"[BUSCAR] ⚠️ Canal no encontrado para el tier "
                f"{solo_tier} (ID {search_channel_id})"
            )
            return await interaction.followup.send(
        ...
        # Mencionamos los rangos incluidos en la ventana de búsqueda.
        ...
        for tier in target_tiers:
            ...
            if role:
                ...
                print(
                    f"[BUSCAR] Mención {tier}: rol={role.name} ({role.id}) "
                    f"mentionable={role.mentionable}"
                )
            else:
                print(
                    f"[BUSCAR] ⚠️ No se encontró el rol SoloQ para {tier}. "
                    f"ID configurado: {role_id}. "
                    f"Servidor: {channel.guild.name} ({channel.guild.id})"
                )

        mention = " ".join(role_mentions)

        me = channel.guild.me
        can_mention = bool(me and me.guild_permissions.mention_everyone)

        print(
            f"[BUSCAR] Publicando SOLO en #{channel.name} "
            f"(tier={solo_tier}, canal={channel.id}) | "
            f"menciones={mention} | bot_mention_everyone={can_mention}"
        )
        ...
        await interaction.followup.send(
            f"✅ Aviso publicado en #{channel.name}. "
